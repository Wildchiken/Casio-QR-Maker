...
bool qrcodegen_encodeSegmentsAdvanced(const struct qrcodegen_Segment segs[], size_t len, enum qrcodegen_Ecc ecl,
    ...
    int version, dataUsedBits, i_ecl, bitLen, dataCapacityBits, terminatorBits;
    size_t i;
    uint8_t padByte;
    ...
    for (i_ecl = (int)qrcodegen_Ecc_MEDIUM; i_ecl <= (int)qrcodegen_Ecc_HIGH; i_ecl++) {
        if (boostEcl && dataUsedBits <= getNumDataCodewords(version, (enum qrcodegen_Ecc)i_ecl) * 8)
            ecl = (enum qrcodegen_Ecc)i_ecl;
    }
    ...
    if (mask == qrcodegen_Mask_AUTO) {
        ...
        int i_msk;
        for (i_msk = 0; i_msk < 8; i_msk++) {
            enum qrcodegen_Mask msk = (enum qrcodegen_Mask)i_msk;
            long penalty;
...
bool qrcodegen_isNumeric(const char *text) {
    for (; *text != '\0'; text++) { if (*text < '0' || *text > '9') return false; }
    return true;
}
bool qrcodegen_isAlphanumeric(const char *text) {
    for (; *text != '\0'; text++) { if (strchr(ALPHANUMERIC_CHARSET, *text) == NULL) return false; }
    return true;
}

size_t qrcodegen_calcSegmentBufferSize(enum qrcodegen_Mode mode, size_t numChars) {
    int temp = calcSegmentBitLength(mode, numChars);
    if (temp == LENGTH_OVERFLOW) return ((size_t)-1);
